...
import malicedev.retroifyer.RetroifyerNetworkMessage;
...
public class BlockLogicRetroifyer extends BlockLogicRotatable {
	private static final Logger LOGGER = LogUtils.getLogger();
	protected Random furnaceRand;
	protected final boolean isActive;
	public static boolean keepFurnaceInventory = false;

	public BlockLogicRetroifyer(Block<?> block, boolean active) {
		super(block, Material.stone);
		this.isActive = active;
		block.withEntity(TileEntityRetroifyer::new);
	}
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int meta, TileEntity tileEntity) {
		switch (dropCause) {
			...
			case SILK_TOUCH:
			case PISTON_CRUSH:
				return new ItemStack[]{new ItemStack(ModBlocks.Retroifyer)};
			default:
				return null;
		}
	}
	...
	public void animationTick(World world, int x, int y, int z, Random rand) {
		if (this.isActive) {
			...
			if (l == 4) {
				...
				world.spawnParticle("flame", poxX - f3, posY, posZ + f4, (double)0.0F, (double)0.0F, (double)0.0F, 0);
			} else if (l == 5) {
				world.spawnParticle("smoke", poxX + f3, posY, posZ + f4, (double)0.0F, (double)0.0F, (double)0.0F, 0);
				world.spawnParticle("flame", poxX + f3, posY, posZ + f4, (double)0.0F, (double)0.0F, (double)0.0F, 0);
			} else if (l == 2) {
				world.spawnParticle("smoke", poxX + f4, posY, posZ - f3, (double)0.0F, (double)0.0F, (double)0.0F, 0);
				world.spawnParticle("flame", poxX + f4, posY, posZ - f3, (double)0.0F, (double)0.0F, (double)0.0F, 0);
			} else if (l == 3) {
				world.spawnParticle("smoke", poxX + f4, posY, posZ + f3, (double)0.0F, (double)0.0F, (double)0.0F, 0);
				world.spawnParticle("flame", poxX + f4, posY, posZ + f3, (double)0.0F, (double)0.0F, (double)0.0F, 0);
			}
	...
	public boolean onBlockRightClicked(World world, int x, int y, int z, Player player, Side side, double xPlaced, double yPlaced) {
		if (!world.isClientSide) {
			NetworkHandler.sendToPlayer(player, new RetroifyerNetworkMessage(x,y,z));
		}
