...
public class NormalHourglassItem extends Item implements IDispensable {
	...
	public void onUseByActivator(ItemStack itemStack, TileEntityActivator activatorBlock, World world, Random random, TilePosc tilePosc, double offX, double offY, double offZ, Direction direction) {
		ProjectileHourglassNormal projectileHourglassNormal = new ProjectileHourglassNormal(world, (double)tilePosc.x() + offX, (double)tilePosc.y() + offY, (double)tilePosc.z() + offZ);
		projectileHourglassNormal.setHeading((double)direction.offsetX() * 0.6, direction.offsetY() == 0 ? 0.1 : (double)direction.offsetY() * 0.6, (double)((float)direction.offsetZ() * 0.6F), 1.1F, 6.0F);
